...
namespace PS.Plot.FrameBasic.Module_SupportLibs.MoonORM.Controller
{
    ...
    /// 实体载入、添加、修改操作的控制器
    /// </summary>
    /// <typeparam name="T">待操作的实体类</typeparam>
    public abstract class EditBaseController<T> : MQLBaseController
        where T : EntityBase, new() 
    {
        ...
        /// <returns></returns>
        public bool DeleteEntry<V>() where V : MQLBase
        {
            bool result = false;
            using (var db = this.dbFactory.OpenDefalutDataBase())
            {
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public bool DeleteEntryByID<V>(int ID) where V : MQLBase
        {
            bool result = false;
            using (var   db = this.dbFactory.OpenDefalutDataBase())
            {
